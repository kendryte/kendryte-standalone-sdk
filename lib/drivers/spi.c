...
uint32_t spi_set_clk_rate(spi_device_num_t spi_num, uint32_t spi_clk)
{
    ...
    printf("baudr is %d\n", spi_adapter->baudr);
    return sysctl_clock_get_freq(SYSCTL_CLOCK_SPI0 + spi_num) / spi_adapter->baudr;
}
                                  ...
                                  size_t cmd_len, void *rx_buff, size_t rx_len)
{
    ...
    dmac_set_single_mode(dma_receive_channel_num, (void *)(&spi_handle->dr[0]), rx_buff, DMAC_ADDR_NOCHANGE, DMAC_ADDR_INCREMENT,
                           DMAC_MSIZE_1/*DMAC_MSIZE_1*/, DMAC_TRANS_WIDTH_32, rx_len);
    if(cmd_len)
...
void spi_receive_data_normal_dma_zzx(dmac_channel_number_t dma_send_channel_num,
                                  dmac_channel_number_t dma_receive_channel_num,
                                  spi_device_num_t spi_num, spi_chip_select_t chip_select, const void *cmd_buff,
                                  size_t cmd_len, void *rx_buff, size_t rx_len, dmac_burst_trans_length_t burst_num)
{
    configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    ...
    dmac_set_single_mode(dma_receive_channel_num, (void *)(&spi_handle->dr[0]), rx_buff, DMAC_ADDR_NOCHANGE, DMAC_ADDR_INCREMENT,
                           burst_num/*DMAC_MSIZE_1*/, DMAC_TRANS_WIDTH_32, rx_len);
    if(cmd_len)
        dmac_set_single_mode(dma_send_channel_num, cmd_buff, (void *)(&spi_handle->dr[0]), DMAC_ADDR_INCREMENT, DMAC_ADDR_NOCHANGE,
                           DMAC_MSIZE_4, DMAC_TRANS_WIDTH_32, cmd_len);
    if(cmd_len == 0 && spi_get_frame_format(spi_num) == SPI_FF_STANDARD)
...
//           read_buf = &write_cmd[i];
    ...
    if(frame_width == SPI_TRANS_INT)
        spi_receive_data_normal_dma_zzx(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, cmd_buff, cmd_len, rx_buff, v_recv_len, burst_num);
    else
        spi_receive_data_normal_dma_zzx(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, write_cmd, cmd_len, read_buf, v_recv_len, burst_num);
