    ...
    unsigned int keybits)
{
    ...
    if (ret != 0)
        return ret;
    ret = mbedtls_cipher_setkey(&ctx->cipher_ctx, key, keybits,
        MBEDTLS_ENCRYPT);
    if (ret != 0)
        return ret;
    ...
    size_t add_len)
{
    ...
    ret = mbedtls_cipher_update(&ctx->cipher_ctx, ctx->y, 16, ctx->base_ectr,
        &olen);
    if (ret != 0)
    {
        printf("close\n");
    ...
    unsigned char* output)
{
    ...
    while (length > 0)
    {
        ...
        for (i = 16; i > 12; i--)
            if (++ctx->y[i - 1] != 0)
                break;
        ret = mbedtls_cipher_update(&ctx->cipher_ctx, ctx->y, 16, ectr,
            &olen);
        if (ret != 0)
            return ret;
