...
int16_t htpa_get_usroffset(htpa_t* obj)
{
    ...
    maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2, &useroffset_high, 1, 100);
    send[0] = 0x00;
    send[1] = ADR_USEROFFSETLOW;
    maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2, &useroffset_low, 1, 100);
    send[0] = 0x00;
    send[1] = ADR_USEROFFSETFLAG;
    maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2, &useroffset_flag, 1, 100);
...
int htpa_get_to_image(htpa_t* obj, int16_t min_in, int16_t max_in, int16_t* img)
{
    ...
    for(int i=0; i<obj->width*obj->height; ++i)
    {
        img[i] = (((int16_t)obj->v[i] + usroffset) - min_in)/(float)range*255;
    }      
    return 0;
}
