...
static inline uint32_t pow_2(uint32_t b) { return 2 << (b - 1); }
...
static void htpa_print_eeprom(htpa_t *obj) {
    printk("PixCmin:%d, PixCmac:%d\r\n", (int)obj->eeprom.pix_c_min,
           (int)obj->eeprom.pix_c_max);
    printk("GradScale:%d\r\n", obj->eeprom.grad_scale);
    printk("TableNumber:%d\r\n", obj->eeprom.table_number);
    printk("epsilon:%d\r\n", obj->eeprom.epsilon);
    printk("calib MBIT:%d, BIAS:%d, CLK:%d, BPA:%d, PU:%d\r\n",
           obj->eeprom.calib_mbit, obj->eeprom.calib_bias,
           obj->eeprom.calib_clk, obj->eeprom.calib_bpa, obj->eeprom.calib_pu);
...
int htpa_read_reg(htpa_t *obj, uint8_t addr) {
    ...
    int ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, temp, 1,
                                 temp + 1, 1, 100);
    if (ret < 0)
        return ret;
    return (int)temp[1];
...
int htpa_init(htpa_t *obj, i2c_device_number_t i2c_num, uint8_t scl_pin,
              uint8_t sda_pin, uint32_t i2c_freq) {
...
int htpa_get_eeprom_param(htpa_t *obj) {
    ...
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)obj->eeprom.vdd_comp_grad, 512, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x05;
    send[1] = 0x40;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)obj->eeprom.vdd_comp_off, 512, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x07;
    send[1] = 0x40;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)obj->eeprom.th_grad, 1024, 100);
    if (ret < 0)
        return ret;
    for (uint8_t i = 0; i < obj->height / 2; ++i) {
        ...
        ret = maix_i2c_recv_data(
            obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
            ((uint8_t *)obj->eeprom.th_grad) + 1024 + 64 * i, 64, 100);
        if (ret < 0)
            return ret;
    }
    send[0] = 0x0F;
    send[1] = 0x40;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)obj->eeprom.th_off, 1024, 100);
    if (ret < 0)
        return ret;
    for (uint8_t i = 0; i < obj->height / 2; ++i) {
        addr = 0x1740 - 64 * (i + 1);
        ...
        ret = maix_i2c_recv_data(
            obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
            ((uint8_t *)obj->eeprom.th_off) + 1024 + 64 * i, 64, 100);
        if (ret < 0)
            return ret;
    }
    send[0] = 0x17;
    ...
    for (uint8_t i = 0; i < obj->height / 2; ++i) {
        ...
        send[1] = ((uint8_t *)&addr)[0];
        ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                                 ((uint8_t *)obj->eeprom.p) + 1024 + 64 * i, 64,
                                 100);
        if (ret < 0)
            return ret;
    }
    ...
    send[1] = 0x00;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.pix_c_min, 4, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x04;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.pix_c_max, 4, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x08;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.grad_scale, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x0B;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.table_number, 2, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x0D;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.epsilon, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x1A;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.calib_mbit, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x1B;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.calib_bias, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x1C;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.calib_clk, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x1D;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.calib_bpa, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x1E;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.calib_pu, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x22;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.array_type, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x26;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.vdd_th1, 2, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x28;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.vdd_th2, 2, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x34;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.ptat_grad, 4, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x38;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.ptat_off, 4, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x3C;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.ptat_th1, 2, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x3E;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.ptat_th2, 2, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x4E;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.vdd_sc_grad, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x4F;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.vdd_sc_off, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x54;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.global_off, 1, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x55;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.global_gain, 2, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x74;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.device_id, 4, 100);
    if (ret < 0)
        return ret;
    send[0] = 0x00;
    send[1] = 0x7F;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                             (uint8_t *)&obj->eeprom.nr_of_def_pix, 1, 100);
    if (ret < 0)
        return ret;
    ...
    for (i = 0; i < 1024; ++i) {
        obj->pix_c[i] =
            ((float)obj->eeprom.p[i] *
                 (obj->eeprom.pix_c_max - obj->eeprom.pix_c_min) / 65535.0 +
             obj->eeprom.pix_c_min) *
                (obj->eeprom.epsilon / 100.0) *
                (obj->eeprom.global_gain / 10000.0) +
            0.5;
    }
...
int htpa_get_electrical_offsets(htpa_t *obj) {
    ...
    while (1) {
        ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR, temp, 1, temp + 1, 1,
                                 100);
        if (ret < 0)
            return 2;
        if ((temp[1] & 0x01) == 0x01)
    ...
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR, temp, 1,
                             (uint8_t *)obj->elec_off, 258, 200);
    if (ret < 0)
        return 3;
    temp[0] = HTPA_CMD_ADDR_DATA2;
    ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR, temp, 1, obj->temp, 258,
                             200);
    if (ret < 0)
        return 4;
    index = 0;
    ...
    {
        obj->elec_off[index++] = ((uint16_t)((uint8_t *)obj->elec_off)[i * 2])
                                     << 8 |
                                 ((uint8_t *)obj->elec_off)[i * 2 + 1];
    }
    for (i = 0; i < 4; ++i) {
        k = 3 - i;
        for (j = 1; j < obj->width + 1; ++j) {
            obj->elec_off[index++] =
                ((uint16_t)((uint8_t *)obj->temp)[k * 64 + j * 2]) << 8 |
                ((uint8_t *)obj->temp)[k * 64 + j * 2 + 1];
        }
    }
    return 0;
}
...
int htpa_get_pixels(htpa_t *obj, bool is_get_vdd) {
    ...
    for (i = 0; i < 4; ++i) {
        ...
        temp[0] = HTPA_CMD_ADDR_STATUS;
        while (1) {
            ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR, temp, 1, temp + 1,
                                     1, 100);
            if (ret < 0)
                return EIO;
            if ((temp[1] & 0x01) == 0x01)
        ...
        if (ret < 0)
            return EIO;
        temp[0] = HTPA_CMD_ADDR_DATA2;
        ret = maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR, temp, 1, obj->temp,
                                 258, 200);
        if (ret < 0)
            return EIO;
        ...
        for (j = 0; j < 4; ++j) {
            ...
            for (col = 1; col < 33; ++col) {
                obj->v[511 + 32 * (k * 4 + k2) + col] =
                    ((uint16_t)(obj->temp)[j * 64 + 2 * col]) << 8 |
                    (obj->temp)[j * 64 + 2 * col + 1];
            }
        }
        if (is_get_vdd) {
            obj->vdd[i] = ((uint16_t)p[0]) << 8 | p[1];
            obj->vdd[7 - i] = ((uint16_t)((uint8_t *)obj->temp)[0]) << 8 |
                              ((uint8_t *)obj->temp)[1];
        } else {
            obj->ptats[i] = ((uint16_t)p[0]) << 8 | p[1];
            obj->ptats[7 - i] = ((uint16_t)((uint8_t *)obj->temp)[0]) << 8 |
                                ((uint8_t *)obj->temp)[1];
        }
    }
    if (is_get_vdd) {
        ...
        obj->ta =
            obj->ptat_mean * obj->eeprom.ptat_grad + obj->eeprom.ptat_off + 0.5;
        // for(int i=0; i<8; ++i)
...
int htpa_get_vdd(htpa_t *obj) {
    ...
    for (i = 0; i < 4; ++i) {
        ...
        temp[0] = HTPA_CMD_ADDR_DATA1;
        ret =
            maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR, temp, 1, temp, 2, 200);
        if (ret < 0)
            return EIO;
...
static inline void thermal_compensation(htpa_t *obj) {
    for (int i = 0; i < 1024; ++i) {
        obj->v[i] = obj->v[i] -
                    (obj->eeprom.th_grad[i] * obj->ptat_mean) /
                        obj->eeprom.grad_scale_div -
                    obj->eeprom.th_off[i];
    }
}
...
static inline void vdd_compensation(htpa_t *obj) {
    int i;
    for (i = 0; i < 512; ++i) {
        // printk("vdd comp: %d %d %d %d %d %d %d %d -- %d %d %d %d %d %d :
        // %d\r\n", obj->v[i], obj->eeprom.vdd_comp_grad[i%128],
        // (int)obj->ptat_mean, obj->eeprom.vdd_sc_grad,
        // obj->eeprom.vdd_sc_grad_div,
        //                    obj->eeprom.vdd_comp_off[i%128],
        //                    obj->eeprom.vdd_sc_off,
        //                    obj->eeprom.vdd_sc_off_div, (int)obj->vdd_mean,
        //                    obj->eeprom.vdd_th1, obj->eeprom.vdd_th2,
        //                    obj->eeprom.vdd_th1, obj->eeprom.ptat_th2,
        //                    obj->eeprom.ptat_th1, (int)((
        //                    obj->eeprom.vdd_comp_grad[i%128] * obj->ptat_mean
        //                    /  obj->eeprom.vdd_sc_grad_div +
        //                    obj->eeprom.vdd_comp_off[i%128] ) /
        ...
        //                       (obj->eeprom.vdd_th2 -
        //                       obj->eeprom.vdd_th1)/(obj->eeprom.ptat_th2 -
        //                       obj->eeprom.ptat_th1) * (obj->ptat_mean -
        //                       obj->eeprom.ptat_th1)
        //                    )));
        obj->v[i] -= (obj->eeprom.vdd_comp_grad[i % 128] * obj->ptat_mean /
                          obj->eeprom.vdd_sc_grad_div +
                      obj->eeprom.vdd_comp_off[i % 128]) /
                     obj->eeprom.vdd_sc_off_div *
                     (obj->vdd_mean - obj->eeprom.vdd_th1 -
                      (obj->eeprom.vdd_th2 - obj->eeprom.vdd_th1) /
                          (obj->eeprom.ptat_th2 - obj->eeprom.ptat_th1) *
                          (obj->ptat_mean - obj->eeprom.ptat_th1));
        // printk("%d\r\n", obj->v[i]);
    }
    for (i = 512; i < 1024; ++i) {
        obj->v[i] -= (obj->eeprom.vdd_comp_grad[i % 128 + 128] *
                          obj->ptat_mean / obj->eeprom.vdd_sc_grad_div +
                      obj->eeprom.vdd_comp_off[i % 128 + 128]) /
                     obj->eeprom.vdd_sc_off_div *
                     (obj->vdd_mean - obj->eeprom.vdd_th1 -
                      (obj->eeprom.vdd_th2 - obj->eeprom.vdd_th1) /
                          (obj->eeprom.ptat_th2 - obj->eeprom.ptat_th1) *
...
static inline void sensitivity_compensation(htpa_t *obj) {
    int i;
    // printk("pix_c:%d %d %d %d\r\n", (int)obj->pix_c[0], (int)obj->pix_c[1],
    // obj->pixels[0], obj->pixels[1]);
    for (i = 0; i < 1024; ++i) {
        obj->v[i] =
            (int32_t)((int64_t)obj->v[i] * HTPA_PCSCALEVAL / obj->pix_c[i]);
    }
    // printk("pix_c:%d %d %d %d\r\n", (int)obj->pix_c[0], (int)obj->pix_c[1],
    // obj->pixels[0], obj->pixels[1]);
}
...
static inline void lookup_table(htpa_t *obj) {
    ...
    for (int m = 0; m < 32; m++) {
        for (int n = 0; n < 32; n++) {
            ...
            vx = ((((int32_t)table_temp[table_row][table_col + 1] -
                    (int32_t)table_temp[table_row][table_col]) *
                   (int32_t)dTA) /
                  (int32_t)HTPA_TAEQUIDISTANCE) +
                 (int32_t)table_temp[table_row][table_col];
            vy = ((((int32_t)table_temp[table_row + 1][table_col + 1] -
                    (int32_t)table_temp[table_row + 1][table_col]) *
                   (int32_t)dTA) /
                  (int32_t)HTPA_TAEQUIDISTANCE) +
                 (int32_t)table_temp[table_row + 1][table_col];
            // obj->v[m*32+n] = (uint32_t)((vy - vx) * ((int32_t)(obj->v[m*32+n]
            // + HTPA_TABLEOFFSET) - (int32_t)table_y_ad_values[table_row]) /
            // ydist + (int32_t)vx);
            obj->v[m * 32 + n] =
                (uint32_t)(
                    (vy - vx) *
                        ((int32_t)(obj->v[m * 32 + n] + HTPA_TABLEOFFSET) -
                         (int32_t)table_y_ad_values[table_row]) /
                        ydist +
                    (int32_t)vx) +
                obj->eeprom.global_off;
            // if(m==0 &&(n==0 || n==1))
            //     printk("x:%d, y:%d, %d %d %d %d vx:%d, vy:%d
            //     Tobject:%d\r\n",table_row, table_col,
            //     table_temp[table_row][table_col],
            //     table_temp[table_row][table_col + 1],
            //     table_temp[table_row+1][table_col], table_temp[table_row +
            //     1][table_col + 1], vx, vy, obj->v[m*32+n]);
            // convert to ℃*100
...
int16_t htpa_get_usroffset(htpa_t *obj) {
    int16_t t_usroffset = 0;
    int16_t ret = 0;
    uint8_t useroffset_flag = 0;
    uint8_t useroffset_high = 0;
    uint8_t useroffset_low = 0;
    uint8_t send[2];
    send[0] = 0x00;
    send[1] = ADR_USEROFFSETHIGH;
    maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                       &useroffset_high, 1, 100);
    send[0] = 0x00;
    send[1] = ADR_USEROFFSETLOW;
    maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2, &useroffset_low,
                       1, 100);
    send[0] = 0x00;
    send[1] = ADR_USEROFFSETFLAG;
    maix_i2c_recv_data(obj->i2c_num, HTPA_ADDR_EEPROM, send, 2,
                       &useroffset_flag, 1, 100);
    t_usroffset = (((int16_t)useroffset_high) << 8) & 0xFF00;
    t_usroffset |= ((int16_t)useroffset_low) & 0xFF;
    t_usroffset += ALGORITHM_OFFSET;
    if (useroffset_flag != OFFSETFLAG) {
        return 0;
    } else {
        return t_usroffset;
    }
...
// get width output
// input: htpa_t htpa_obj, uint16_t width
int htpa_width(htpa_t *obj, uint16_t *width) {
    *width = obj->width;
    return 0;
}
// get height output
// input: htpa_t htpa_obj, uint16_t height
