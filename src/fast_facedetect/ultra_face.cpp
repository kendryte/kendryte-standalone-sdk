...
/* mix nms was been proposaled in paper blaze face, aims to minimize the temporal jitter*/
        ...
        for (int i = 0; i < num_anchors; i++) {
            if (scores[i * 2 + 1] > score_threshold) {
                ...
                float x_center = boxes[i * 4] * center_variance * priors[i][2] +
                                 priors[i][0];
                float y_center =
                    boxes[i * 4 + 1] * center_variance * priors[i][3] +
                    priors[i][1];
...
int ultra_face_init(int input_width, int input_height, float score_threshold,
    ...
    g_detector = std::make_unique<ultra_face>(
        input_width, input_height, score_threshold, iou_threshold, topk);
    return 0;
}
...
int ultra_face_detect(float *scores, float *boxes, callback_draw_box callback) {
    ...
    for (auto &face : output) {
        ...
        // face.score);
        callback((int)roundf(face.x1), (int)roundf(face.y1),
                 (int)roundf(face.x2), (int)roundf(face.y2), face.score);
    }
