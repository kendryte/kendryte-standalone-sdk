...
w25qxx_status_t test_w25qxx_write_data(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    uint32_t sector_addr = 0;
    uint32_t sector_offset = 0;
    uint32_t sector_remain = 0;
    ...
    while (length)
    {
        ...
        sector_remain = w25qxx_FLASH_SECTOR_SIZE - sector_offset;
        write_len = ((length < sector_remain) ? length : sector_remain);
        w25qxx_read_data(sector_addr, swap_buf, w25qxx_FLASH_SECTOR_SIZE, W25QXX_STANDARD);
...
uint8_t buf0[TEST_LEN];
uint8_t buf1[TEST_LEN];
void w25qxx_test(void)
{
	uint32_t addr = 0x1000000-0x1000-0x500;	//last 4KB
    uint32_t count = 0;
    uint32_t error = 0;
	...
	while(1){
		test_w25qxx_write_data(addr, buf0, TEST_LEN);
		w25qxx_read_data(addr, buf1, TEST_LEN, W25QXX_STANDARD);
		i0 = -1;
		i1 = -1;
		#if 1
		for(i=0; i<TEST_LEN; i++)
